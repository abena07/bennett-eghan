...
import "swiper/css";
import { publrPhotosUrl, toPlaceholderUrl } from "@/lib/publr";
...
function PhotoLightbox({ urls, initialIndex, onClose }: PhotoLightboxProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-3 pb-10 pt-14 sm:px-10 sm:pb-12">
        <div className="photo-lightbox-swiper pointer-events-auto w-full max-w-[min(96vw,1200px)] max-h-[min(85vh,880px)] [--swiper-theme-color:rgb(255_255_255/0.35)] [&_.swiper]:outline-none [&_.swiper-wrapper]:outline-none [&_.swiper-slide]:outline-none">
          <Swiper
            autoHeight
...
export function PhotosGallery({
  ...
  const [photos, setPhotos] = useState<string[] | null>(
    initialPhotos.length > 0 ? initialPhotos : userId ? null : [],
  );
  ...
  // Fallback client-side fetch, only when the server didn't already provide photos.
  useEffect(() => {
    if (!userId || initialPhotos.length > 0) return;
  ...
  const teardownMasonryEffects = useCallback(() => {
    ...
    gridRef.current?.querySelectorAll(".publr-photo-tile").forEach((el) => {
      el.classList.remove(
        "publr-photo-shown",
        "publr-photo-animate",
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Dynamically imported: masonry-layout's UMD wrapper references the
      // `window` global at module scope, which breaks server-side rendering.
      const { default: MasonryCtor } = await import("masonry-layout");
      const masonry = new MasonryCtor(grid, {
      ...
      window.requestAnimationFrame(() => {
        ...
        const tiles = gridRef.current.querySelectorAll<HTMLLIElement>(
          ".publr-photo-tile",
        );
        ...
        const io = new IntersectionObserver(
          (entries) => {
            for (const entry of entries) {
              ...
              const durSeconds =
                Math.random() * (DEMO2_MAX_DURATION - DEMO2_MIN_DURATION) +
                DEMO2_MIN_DURATION;
      ...
      return (
        <>
          ...
          <code className="rounded-sm bg-[#E1E4EA]/70 px-1 py-0.5 text-[11px]">
            NEXT_PUBLIC_PUBLR_USER_ID=&lt;your id&gt;
          </code>{" "}
        ...
        {photos !== null && photos.length > 0 ? (
          <ul ref={gridRef} className="publr-masonry effect-2 w-full pb-24">
            ...
            {photos.map((url, index) => (
              <li key={`${index}-${url}`} className="publr-photo-tile">
                <PhotoTile
                  url={url}
                  index={index}
          ...
          <p className="text-[13px] text-[#0B0F1F]/70">no photos yet.</p>
        ) : null}
      </motion.div>

      {lightbox !== null && photos !== null && photos.length > 0 ? (
        <PhotoLightbox
          key={lightbox.id}
          urls={photos}
