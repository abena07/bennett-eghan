...
you don’t need to master the call stack right now. but knowing that the computer is stacking and unstacking function calls helps make sense of how recursive functions behave.

> side note: this is explained super well in *a common-sense guide to data structures and algorithms (vol. 1)* by jay wengrow [\[1\]](#ref1). highly recommend.
...
since our data type here is a list, the smallest version is when the list is empty or has just one item.
...
thinking recursively doesn't come easy to most people (me included), so a good way to get into the mindset is:
...
fun fact, i genuinely think recursion was created just to humble programmers once in a while 😂
